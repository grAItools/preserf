...
- The opencode command workflows were hardened after review:
  ...
  token is followed by a space or ends the comment (previously a bare
  `contains()` let any commenter start a secrets-bearing run via a substring
  like `/ocaml`), grants `contents: read` so `actions/checkout` can read the
  repo, pins `actions/checkout` to `v6.0.3` (matching the other workflows), uses
  a consistent step indentation, and resolves its model via a valid `vars.*`
  reference with a fallback instead of the non-existent `variables` context. `.github/workflows/opencode-cmd-engine.yml` restores
