...
The governing rule, recorded in
[`../docs/adr/0007-agents-md-single-source-of-truth.md`](../docs/adr/0007-agents-md-single-source-of-truth.md):
**root [`AGENTS.md`](../AGENTS.md) is the single source of truth for agent
instructions. Wire every agent to it by reference — never duplicate content.**
...
3. **Needs a config key or a frontmatter'd file?** Add a thin pointer/config stub
   that references `AGENTS.md` (precedents: `.github/copilot-instructions.md`,
   `.gemini/settings.json`). Never copy instruction prose into it.
4. Put tool-specific guidance (not meant for every agent) in that tool's own file,
   not in `AGENTS.md`.

## Caveats

- **Copier-managed.** This harness is generated from
  ...
  port durable changes upstream behind a per-agent toggle. Net-new files are safe.
