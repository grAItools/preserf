...
`preserf` ships a single agentic coding harness that both **Claude
...
- **SessionStart** runs [`.agents/hooks/ensure-toolchain.sh`](../.agents/hooks/ensure-toolchain.sh)
  to **install** `pixi` if it is missing. The installer adds it
...
- **Behaviour baked in:** works one phase at a time; **writes the failing test
  ...
  `pixi run verify` at every phase boundary; ticks `tasks.md` in the same
...
- **verify skill** = "run `pixi run verify`, triage failures, propose
