...
!> `fs_serialization_status`, plus the tracer + k-buffer surface (Slice C
!> / ADR 0003): `fs_RegisterAllTracers`, `ppser_write_tracer_by_name`,
!> `ppser_write_tracer_by_idx`, `ppser_write_tracer_all`, the host-side
!> `ppser_register_tracer`, and `fs_write_kbuff` (DATA_KBUFF).
!>
!> pp_ser output that uses `OPTION` imports `fs_Option`, which `m_preserf`
!> does NOT yet export (Slice C Phase 3), and will fail to compile against
!> this re-export until that follow-up lands.
module m_serialize
   use m_preserf
   implicit none
